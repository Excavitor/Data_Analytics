...
export class InventoryItemTime1Component implements OnInit {
  ...
  processChartData(): void {
    for (const d of this.data_all['div_q1']) {
      this.months.push(d.quarter);
      this.sales1.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q2']) {
      this.sales2.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q3']) {
      this.sales3.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q4']) {
      this.sales4.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q5']) {
      this.sales5.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q6']) {
      this.sales6.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q7']) {
      this.sales7.push(d.stock_quantity);
    }

    for (const d of this.data_all['div_q8']) {
      this.sales8.push(d.stock_quantity);
    }
